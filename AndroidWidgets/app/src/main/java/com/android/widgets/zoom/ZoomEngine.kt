...
internal constructor(context: Context) : ZoomApi {
    ...
    // Options & state
    private var transformationType = TRANSFORMATION_CENTER_INSIDE
    private var transformationGravity = TRANSFORMATION_GRAVITY_AUTO
     ...
     */
    @RealZoom
    override val realZoom
        get() = matrixController.zoom
    ...
    @AbsolutePan
    val contentWidth
        get() = matrixController.contentWidth
    ...
    @AbsolutePan
    val contentHeight
        get() = matrixController.contentHeight
    ...
    /**
     * Sets the content alignment. Can be any of the constants defined in [ZoomAlignment].
     ...
     */
    override fun setAlignment(@Alignment alignment: Int) {
     ...
     * which might also be [TRANSFORMATION_GRAVITY_AUTO]. In this case we should
    ...
    private fun computeTransformationGravity(input: Int): Int {
        return when (input) {
            TRANSFORMATION_GRAVITY_AUTO -> {
                val horizontal = ZoomAlignment.toHorizontalGravity(panManager.alignment, Gravity.CENTER_HORIZONTAL)
                val vertical = ZoomAlignment.toVerticalGravity(panManager.alignment, Gravity.CENTER_VERTICAL)
     ...
     * Zoom might not be the actual matrix scale, see [zoom] and [realZoom].
     ...
     */
    override fun moveTo(@Zoom zoom: Float,
                        @AbsolutePan x: Float,
                        @AbsolutePan y: Float,
                        animate: Boolean) {
